                  ...
                  <div className="flex items-center gap-2">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        <DropdownMenuItem onClick={(e)=>{e.preventDefault();copyCitation('bibtex',selectedPublication);}}><Copy className="h-4 w-4 mr-2"/>Copy BibTeX</DropdownMenuItem>
                        <DropdownMenuItem onClick={(e)=>{e.preventDefault();copyCitation('apa',selectedPublication);}}><Copy className="h-4 w-4 mr-2"/>Copy APA</DropdownMenuItem>
                        <DropdownMenuItem onClick={(e)=>{e.preventDefault();copyCitation('mla',selectedPublication);}}><Copy className="h-4 w-4 mr-2"/>Copy MLA</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
...
export function Publications() {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>("grid");
  const modalRef = useRef<HTMLElement>(null);
  // Parse categories supporting either a comma-separated string or an array
  ...
  // Helper for publication object (supports future categories[] field)
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        <DropdownMenuItem onClick={(e) => { e.preventDefault(); copyCitation('bibtex', selectedPublication); }}>
                          <Copy className="h-4 w-4 mr-2" /> Copy BibTeX
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={(e) => { e.preventDefault(); copyCitation('apa', selectedPublication); }}>
                          <Copy className="h-4 w-4 mr-2" /> Copy APA
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={(e) => { e.preventDefault(); copyCitation('mla', selectedPublication); }}>
                          <Copy className="h-4 w-4 mr-2" /> Copy MLA
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button variant="outline" size="sm" onClick={() => sharePaper(selectedPublication)} aria-label="Share publication">
              ...
              <div className="space-y-6 mt-6">
                  ...
                  <div className="relative rounded-xl overflow-hidden shadow-lg border-4 border-yellow-300">
                    <div className="absolute top-4 left-4 z-10">
                      <div className="bg-gradient-to-br from-yellow-400 to-orange-500 text-white px-4 py-2 rounded-full shadow-lg flex items-center gap-2">
                        <Award className="h-5 w-5" /> <span>Award Certificate</span>
                      </div>
                    </div>
                    <ImageWithFallback
                ...
                <div>
                  <div className="flex items-center gap-2 text-slate-900 dark:text-slate-50 mb-3">
                    <Users className="h-5 w-5 text-violet-600 dark:text-violet-400" /> <span>Authors</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {selectedPublication.author_ids?.map((entry: any) => {
                      ...
                      return (
                        <motion.button
                          ...
                          whileTap={{ scale: 0.95 }}
                          onClick={(e) => { e.stopPropagation(); if (rawId != null) handleAuthorClick(rawId); }}
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-blue-50 dark:bg-blue-950/30 rounded-lg p-4">
                    <div className="flex items-center gap-2 text-blue-900 dark:text-blue-300 mb-1"><Calendar className="h-4 w-4" /> <span>Year</span></div>
                    <p className="text-sm text-blue-800 dark:text-blue-400">{selectedPublication.year}</p>
                  </div>
                  {selectedPublication.publisher && (
                    <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-4">
                      <div className="flex items-center gap-2 text-slate-900 dark:text-slate-300 mb-1"><Building2 className="h-4 w-4" /> <span>Publisher</span></div>
                      <p className="text-sm text-slate-800 dark:text-slate-400">{selectedPublication.publisher}</p>
                    </div>
                  )}
                  {selectedPublication.conference && (
                    <div className="bg-violet-50 dark:bg-violet-950/30 rounded-lg p-4">
                      <div className="flex items-center gap-2 text-violet-900 dark:text-violet-300 mb-1"><Building2 className="h-4 w-4" /> <span>Conference</span></div>
                      <p className="text-sm text-violet-800 dark:text-violet-400">{selectedPublication.conference}</p>
                    </div>
                  )}
                  {selectedPublication.journal && (
                    <div className="bg-blue-50 dark:bg-blue-950/30 rounded-lg p-4">
                      <div className="flex items-center gap-2 text-blue-900 dark:text-blue-300 mb-1"><BookOpen className="h-4 w-4" /> <span>Journal</span></div>
                      <p className="text-sm text-blue-800 dark:text-blue-400">{selectedPublication.journal}</p>
                    </div>
                  )}
                  {selectedPublication.book_chapter && (
                    <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-4">
                      <div className="flex items-center gap-2 text-slate-900 dark:text-slate-300 mb-1"><FileText className="h-4 w-4" /> <span>Book Chapter</span></div>
                      <p className="text-sm text-slate-800 dark:text-slate-400">{selectedPublication.book_chapter}</p>
                    </div>
                  )}
                  <div className="bg-violet-50 dark:bg-violet-950/30 rounded-lg p-4">
                    <div className="flex items-center gap-2 text-violet-900 dark:text-violet-300 mb-1"><TrendingUp className="h-4 w-4" /> <span>Citations</span></div>
                ...
                <div className="bg-blue-50 dark:bg-blue-950/30 rounded-lg p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {getPublicationCategories(selectedPublication).map(cat => (
                      <Badge key={cat} variant="outline" className="bg-white dark:bg-slate-800 border-blue-200 dark:border-blue-800">{cat}</Badge>
                    ))}
                    {getPublicationCategories(selectedPublication).length === 0 && (
                      <Badge variant="outline" className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400">Uncategorised</Badge>
                    )}
                  </div>
                </div>
                ...
                {selectedPublication.abstract && (
                  <div className="bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-800 dark:to-blue-950/30 rounded-lg p-6 border border-slate-200 dark:border-slate-700">
                    <div className="flex items-center gap-2 text-slate-900 dark:text-slate-50 mb-3"><FileText className="h-5 w-5 text-slate-600 dark:text-slate-400" /> <span>Abstract</span></div>
                ...
                {selectedPublication.url && (
                  ...
                  >
                    <motion.div
                      ...
                      tabIndex={0}
                    >
                      <BookOpen className="h-5 w-5" /> <span>View Publication</span> <ExternalLink className="h-4 w-4" />
                    </motion.div>
                  </a>
                )}
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      <div>
                        <h3 className="text-lg mb-4 flex items-center gap-2"><Sparkles className="h-5 w-5 text-purple-600 dark:text-purple-400" /> Related Publications</h3>
                        <div className="grid gap-3">
                          {related.map(relPub => (
                            <motion.div
                              key={relPub.id}
                              whileHover={{ x: 4 }}
                              onClick={(e) => { e.stopPropagation(); setSelectedPublication(relPub); }}
                              role="button"
                              tabIndex={0}
                              aria-label={`View ${relPub.title}`}
                              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); setSelectedPublication(relPub); } }}
                            ...
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div className="flex-1 min-w-0">
                                  <h4 className="text-sm group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors line-clamp-2 mb-1">{relPub.title}</h4>
                                  <div className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
                                    <Calendar className="h-3 w-3" /> <span>{relPub.year}</span> <span>•</span> <TrendingUp className="h-3 w-3 text-green-500" /> <span>{relPub.citations} citations</span>
                                  </div>
                                </div>
            ...
            {filteredPublications.map((pub, index) => (
              ...
              <Card className="overflow-hidden h-full flex flex-col hover:shadow-2xl dark:hover:shadow-purple-900/20 transition-all duration-300 group bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
                ...
                <CardHeader className="flex-1">
                  ...
                  <CardDescription className="flex items-start gap-2 text-slate-600 dark:text-slate-400">
                    <Users className="h-4 w-4 shrink-0 mt-0.5" />
                    <span className="line-clamp-2 whitespace-pre-wrap">{pub.authors}</span>
                  </CardDescription>
                </CardHeader>

                <CardContent className="space-y-3">
                  {/* Venue and Publisher on same line */}
                  <div className="flex items-start justify-between gap-2 text-sm text-slate-600 dark:text-slate-400">
                    <div className="flex items-start gap-2 flex-1 min-w-0">
                      <Building2 className="h-4 w-4 shrink-0 mt-0.5 text-violet-500 dark:text-violet-400" />
                  ...
                  {/* Citations removed from card views; shown only in detail modal */}

                  {/* Category Badges (centered) */}
                  <div className="flex flex-wrap gap-2 justify-center">
                    {getPublicationCategories(pub).map(cat => (
                      <Badge
                        key={cat}
                        variant="outline"
                        className="bg-violet-50 dark:bg-violet-950/30 border-violet-200 dark:border-violet-800 text-violet-700 dark:text-violet-400"
                      >
                        {cat}
                      </Badge>
                    ))}
                    {getPublicationCategories(pub).length === 0 && (
                      <Badge
                        variant="outline"
                ...
                <Card className="cursor-pointer hover:shadow-lg dark:hover:shadow-purple-900/20 transition-all duration-300 group bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="flex-1 p-6">
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap gap-2 mb-3">
                            ...
                            {parseCategories(pub.category).map(cat => (
                              <Badge key={cat} variant="outline" className="bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400">
                                {cat}
                              </Badge>
                            ))}
                            {parseCategories(pub.category).length === 0 && (
                              <Badge variant="outline" className="bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400">Uncategorised</Badge>
                            )}
                            {pub.tag === "Featured" && (
                        ...
                        {/* Citations badge removed from list cards */}
                      </div>
                      ...
                      {/* Venue and Publisher on same line */}
                      <div className="flex items-start justify-between gap-2 text-sm text-slate-600 dark:text-slate-400 mb-2">
                        <div className="flex items-start gap-2 flex-1 min-w-0">
                          <Building2 className="h-4 w-4 shrink-0 mt-0.5 text-violet-500 dark:text-violet-400" aria-hidden="true" />
                          <span className="italic line-clamp-1 flex-1">
                            {pub.venue || "Venue not provided"}
                          </span>
                        </div>
                      ...
                      {/* Citations removed from list cards */}
                        ...
                        <Card className="h-full hover:shadow-lg dark:hover:shadow-purple-900/20 transition-all duration-300 group bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
                          ...
                          <CardContent className="space-y-2">
                            <div className="flex flex-wrap gap-2">
                              ...
                              {parseCategories(pub.category).map(cat => (
                                <Badge key={cat} variant="outline" className="text-xs bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400">
                                  {cat}
                                </Badge>
                              ))}
                              {parseCategories(pub.category).length === 0 && (
                                <Badge variant="outline" className="text-xs bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400">Uncategorised</Badge>
                              )}
                              {pub.tag === "Featured" && (
                            ...
                            {/* Venue and Publisher on same line */}
                            <div className="flex items-start justify-between gap-2 text-xs text-slate-600 dark:text-slate-400">
                              <div className="flex items-start gap-2 flex-1 min-w-0">
                                <Building2 className="h-3 w-3 shrink-0 mt-0.5 text-violet-500 dark:text-violet-400" aria-hidden="true" />
                                <span className="italic line-clamp-1 flex-1">
                                  {pub.venue || "Venue not provided"}
                                </span>
                              </div>
                              {pub.publisher && (
                                <span className="text-[10px] font-semibold text-violet-600 dark:text-violet-400 shrink-0 ml-1">
                                  {pub.publisher}
                                </span>
                              )}
                            </div>
                            {/* Citations removed from timeline cards */}
                          </CardContent>
                        </Card>
                      </motion.div>
                    ))}
